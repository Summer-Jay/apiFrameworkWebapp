...
public class Test {
	...
	public Test(HttpStatus httpStatus){
		HTTP_HEADER = new HttpHeaders();
		HTTP_HEADER.setContentType(new MediaType("application", "json", Charset.forName("UTF-8")));
		STATUS=httpStatus;
	}

	public static Test getBuilderInstance(HttpServletRequest request) {
		Test resManager = new Test(HttpStatus.OK);
	...
	public static ResponseEntity<Map<String, Object>> getResult(Test resManager){
		if(resManager != null) {

		}else {
			resManager = new Test(HttpStatus.BAD_REQUEST);
